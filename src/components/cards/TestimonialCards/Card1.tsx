...
const Card1: React.FC<Testimonial> = ({
  rating,
  content,
  author,
  position,
  ...
  return (
    <div className="group bg-white border border-solid border-gray-300 rounded-2xl p-6 transition-all duration-500 w-full hover:border-indigo-600">
      ...
      <div className="flex items-center gap-5">
        <img className="rounded-full object-cover" src={avatar} alt="avatar" />
        <div className="grid gap-1">
          <h5 className="text-gray-900 font-medium transition-all duration-500 group-hover:text-indigo-600">
            {author}
          </h5>
          <span className="text-sm leading-6 text-gray-500">{position}</span>
        </div>
      </div>
    </div>
  );
